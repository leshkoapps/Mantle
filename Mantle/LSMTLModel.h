...
///
/// LSMTLPropertyStorageNone       - This property is not included in -description,
///                                -hash, or anything else.
/// LSMTLPropertyStorageTransitory - This property is included in one-off
///                                operations like -copy and -dictionaryValue but
///                                does not affect -isEqual: or -hash.
///                                It may disappear at any time.
/// LSMTLPropertyStoragePermanent  - The property is included in serialization
///                                (like `NSCoding`) and equality, since it can
///                                be expected to stick around.
typedef enum : NSUInteger {
    LSMTLPropertyStorageNone,
    LSMTLPropertyStorageTransitory,
    LSMTLPropertyStoragePermanent,
} LSMTLPropertyStorage;

/// This protocol defines the minimal interface that classes need to implement to
/// interact with Mantle adapters.
///
/// It is intended for scenarios where inheriting from LSMTLModel is not feasible.
/// However, clients are encouraged to subclass the LSMTLModel class if they can.
///
/// Clients that wish to implement their own adapters should target classes
/// conforming to this protocol rather than subclasses of LSMTLModel to ensure
/// maximum compatibility.
@protocol LSMTLModel <NSObject, NSCopying>
...
/// properties without ivars, or properties on LSMTLModel itself.
+ (NSSet *)propertyKeys;
...
@interface LSMTLModel : NSObject <LSMTLModel>
...
///
/// The default implementation returns LSMTLPropertyStorageNone for properties that
/// are readonly and not backed by an instance variable and
/// LSMTLPropertyStoragePermanent otherwise.
///
/// Subclasses can use this method to prevent LSMTLModel from resolving circular
/// references by returning LSMTLPropertyStorageTransitory.
///
...
/// Implements validation logic for LSMTLModel.
@interface LSMTLModel (Validation)
